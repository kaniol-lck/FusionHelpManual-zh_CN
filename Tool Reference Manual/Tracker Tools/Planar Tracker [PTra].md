...
Stabilize模式用于通过应用部分抵消摄像机抖动的变换来消除相机中的抖动。该稳定变换（包含在Stable Track样条中）是通过平滑相邻帧上的跟踪变换来计算的。注意，Stabilize模式只能使运动平滑，而Steady模式才会尝试完全“锁定”所有运动。
