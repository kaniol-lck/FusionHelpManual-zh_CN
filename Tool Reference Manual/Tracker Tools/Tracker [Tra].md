...
Tracker时用于探测并在移动的视频中跟随一个或多个像素图案。跟踪数据可以在之后用于控制位置或合成中其他工具的值（例如Drip的中心）。另外，跟踪器可以用于稳定图像或基于一个图像的移动来向另一图像应用取消稳定。
...
- **Match Move 匹配移动：**该模式用于稳定图像或匹配图层的位置、旋转及缩放来匹配移动另一元素至场景中。稳定需要至少一个跟踪器来确定位置，两个或多个确定了序列中的缩放和旋转。
...
- **BG Only 仅背景：**忽略前景输入，只影响背景。这主要在稳定图像序列时使用。
...
- **BG over FG 背景覆盖前景：**将背景合并到前景中。这一技巧经常用于当使用Alpha通道跟踪图层来获得稳定的背景来衬在背后。
...
Tracker工具会自动输出都走红稳定和不稳定的位置输出至流程中其他可连接的控件。Stable Position输出提供了X和Y的坐标来在序列中匹配或反转运动。这些控件即使在Operation模式不为Match Move时也可用，因为Stable Postion输出总是可用于与其他工具的连接。

##### Pivot Type 中心轴类型

在几乎所有情况下，用于任意稳定的轴应该是该帧所有跟踪器的平均位置，然而在一些罕见的情况下选哟跟踪器的轴位于别的位置。

该按钮组允许选择基于单个跟踪器的稳定的轴，或手动放置。

##### Reference 参考

Reference控件用于设置稳定的“快照帧”。当你稳定图像时，必须有一个被认为是正确的位置，来检测并校正后续的移动。

- **Select Time 选择时间：**Select Time参考模式确定了基于其中首次选择的图案的帧的“快照帧”。所有的稳定都意在将该图像返回到参考。

- **Start 起始：**“快照帧”将是跟踪路径中的第一帧。所有的稳定都意在将该图像返回到参考。

- **Start and End 起始和结束：**Start and End参考模式与别的模式有一些不同。其中其他的模式意在取得稳定返回的快照帧，而Start and End模式意在平滑现存的运动而不是移除它。该模式平均了路径起始到结束之间的运动，在这些点之间绘制一条直线。

  该模式激活时，它会显露出Reference Intermediate Points控件。增加该控件的值会增加Reference使用的路径上的点的数量，并平滑在起始至结束之间的直线运动使其不完全是线性的。

- **End 结束：**快照帧将是跟踪路径的最后一帧。所有的稳定都意在将该图像返回到参考。
...
- **UnsteadyPosition1：**跟踪器1不稳定位置（与第一个、第二个等类似）