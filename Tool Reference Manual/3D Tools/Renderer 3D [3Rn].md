...
此显示框中的前两个复选框可用于确定该工具是否将在渲染到控制台时打印警告和错误。第二行复选框告诉工具在遇到警告或错误时是否应该中止渲染。此工具的默认设置将勾选所有四个复选框。
...
勾选Enable Lighting复选框后，场景中的任何灯光都将照亮对象。如果没有灯光，所有的物体都是黑色的。

##### Enable Shadows 启用阴影

勾选Enable Shadows复选框后，渲染器将产生阴影，这会有一定的速度代价。
...
取消勾选此复选框时，将显示单独的滑块来控制X轴和y轴上的超级采样。
...
取消勾选此复选框时，可以在设置滤镜宽度时为X轴和Y轴设置不同的值。
